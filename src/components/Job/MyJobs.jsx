...
const MyJobs = () => {
    ...
    const fetchJobs = async () => {
      try {
        const { data } = await axios.get(
          "https://intern-hub-backend.vercel.app/api/v1/job/getmyjobs",
  ...
  const handleUpdateJob = async (jobId) => {
    ...
    await axios
      .put(`https://intern-hub-backend.vercel.app/api/v1/job/update/${jobId}`, updatedJob, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  const handleDeleteJob = async (jobId) => {
    await axios
      .delete(`https://intern-hub-backend.vercel.app/api/v1/job/delete/${jobId}`, {
        withCredentials: true,
      })
      .then((res) => {
        toast.success(res.data.message);
  ...
  return (
    <div className="myJobs page">
      <div className="container">
        <h1>Your Posted Internships</h1>
        {myJobs.length > 0 ? (
          <div className="banner">
            {myJobs.map((element) => (
              <div
                className={`card ${
                  editingMode === element._id ? "full_view" : "minimal_view"
                }`}
                key={element._id}
              >
                <div className="content">
                  <div className="minimal_fields">
                    <div>
                      <span>Title:</span>
                      <input
                        type="text"
    ...
    {/* Add more options as needed */}
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        disabled={editingMode !== element._id}
                        value={element.fixedSalary || element.salaryFrom}
                        onChange={(e) =>
                          handleInputChange(
                            element._id,
                            "fixedSalary" in element
                              ? "fixedSalary"
                              : "salaryFrom",
                            e.target.value
                          )
